...
try
{
	...
	{
		String username,password,bbname,type,city,address,email,phone;
        String id,newid;
		
		username=request.getParameter("txt_username"); //txt_username 
		password=request.getParameter("txt_password"); //txt_password
        bbname=request.getParameter("txt_bbname"); //txt_bbname
		type=request.getParameter("txt_type"); //txt_type
        city=request.getParameter("txt_location"); //txt_location
		...
		st=con.prepareStatement("select * from bblogin where Username=?"); //sql select query 
	 ...
	 if(rs.next()) //if email id already exist
	 {
		...
		pstmt=con.prepareStatement("select * from identity where Class=?");
	    ...
	    {
            id=rst.getString("Id");
            newid="H" + (Integer.parseInt(id.substring(1,id.length()))+1);
        
            pstmt=con.prepareStatement("update identity set Id=? where Class=?"); //sql insert query
            pstmt.setString(1,newid);
            pstmt.setString(2,"HID");
            pstmt.executeUpdate(); //execute query
        
        pstmt=con.prepareStatement("insert into bblogin(id,username,password,bbname,type,city,address,email,phone) values(?,?,?,?,?,?,?,?,?)"); //sql insert query
		...
		pstmt=con.prepareStatement("insert into availability(Id,Username,Apos,Bpos,Opos,ABpos,Aneg,Bneg,Oneg,ABneg) values(?,?,?,?,?,?,?,?,?,?)"); //sql insert query
        pstmt.setString(1,id);
        pstmt.setString(2,username);
		pstmt.setInt(3,zero);
		pstmt.setInt(4,zero);
...
<head>
	...
	<title>Register : Blood Banks</title>

	<link rel="stylesheet" href="css/bbregister.css">
	...
	<script>	
		...
		function validate()
		{
			...
			var bb_name= /^[a-z A-Z]+$/; //pattern allowed alphabet a-z or A-Z 
            ...
            var bbname = document.getElementById("bbn"); //textbox id lname
			...
			if(!bb_name.test(bbname.value) || bbname.value=='') 
            {
				alert("Enter Blood bank name Alphabet Only....!");
                bbname.focus();
                bbname.style.background = '#f08080';
                return false;                    
            }
            ...
            <div class="form-register-with-email">
                ...
                <div class="form-row">
                    <label>
                        ...
                        <input type="text" name="txt_bbname" id="bbn" placeholder="enter blood bank name">
                    </label>
                </div>
                ...
                <div class="form-row">
                    <label>
                        <span>City</span>
                        <select name="txt_location" id="citySelect" size="1">
                ...
                <div class="form-row">
                    <label>
                        <span>Address</span>
                        <input type="text" name="txt_address" id="addr" placeholder="enter full address">
                    </label>
                </div>
                ...
                <a href="bblogin.jsp" class="form-log-in-with-existing">Already have an account? <b> Login here </b></a>
